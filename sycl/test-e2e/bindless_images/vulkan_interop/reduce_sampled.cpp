...
clang++ -fsycl  -DVERBOSE_PRINT=1   -I$VULKAN_SDK/include -L$VULKAN_SDK/lib -lvulkan  -o rs_default.bin reduce_sampled.cpp
NEOReadDebugKeys=1 UseBindlessMode=1 UseExternalAllocatorForSshAndDsh=1 ./rs_default.bin
...
bool run_test(sycl::range<NDims> dims, sycl::range<NDims> localSize,
  ...
  auto inputImageMemoryTypeIndex = vkutil::getImageMemoryTypeIndex(inputImage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, memRequirements);
  // imageSizeBytes is wrong.  
  // VULKAN KNOWS the image size, it is NOT just w * h * bytes  
  auto inputMemory = vkutil::allocateDeviceMemory(/*imageSizeBytes*/ memRequirements.size, inputImageMemoryTypeIndex, inputImage);
  VK_CHECK_CALL(vkBindImageMemory(vk_device, inputImage, inputMemory, 0 /*memoryOffset*/));

  printString("Creating staging buffers\n");
  // Create input staging memory
  auto inputStagingBuffer = vkutil::createBuffer( imageSizeBytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);


  VkMemoryRequirements bufferRequirements;
  vkGetBufferMemoryRequirements(vk_device, inputStagingBuffer, &bufferRequirements);
  ...
  auto inputStagingMemory =
      vkutil::allocateDeviceMemory(/*imageSizeBytes*/ bufferRequirements.size, inputStagingMemoryTypeIndex,
...
// #ifdef TEST_L0_SUPPORTED_VK_FORMAT
  ...
  // hang
  // printString("sycl::half 4D -- fp16_3d_c4");
  ...
  // hang
  // printString("uint8_t 4D -- unorm_int8_3d_c4");
...
// why do these class name choices not match the arity of the test?

  // result mismatch
  printString("float 4D -- float_2d");
  valid &= run_test<2, float, 4, sycl::image_channel_type::fp32,
                    sycl::image_channel_order::rgba, class float_2d>({16, 16},
                                                                     {2, 2}, 0);

  // hang                                                                   
  printString("float 2D -- float_2d_large");
  ...
  // hang
  printString("int8_t 2D -- int8_2d");
  valid &= run_test<3, char, 2, sycl::image_channel_type::signed_int8,
                    sycl::image_channel_order::rg, class int8_2d>({256, 16, 2},
                                                                  {2, 2, 2}, 0);


  // hang  --    UR_RESULT_ERROR_OUT_OF_RESOURCES
  // printString("uint32_t 1D -- uint32_1d");
  // valid &= run_test<2, uint32_t, 1, sycl::image_channel_type::unsigned_int32,
  //                   sycl::image_channel_order::r, class uint32_1d>({64, 32},
  //                                                                  {4, 2}, 0);

    
  // hang  --  UR_RESULT_ERROR_OUT_OF_RESOURCES
  // printString("uint32_t 4D -- uint32_4d_large");
  // valid &= run_test<3, uint32_t, 4, sycl::image_channel_type::unsigned_int32,
  //                   sycl::image_channel_order::rgba, class uint32_4d_large>(
  //     {1024, 256, 16}, {2, 2, 4}, 0);
...
// hang
  // printString("int32_t 1D -- int32_1d");    
  // valid &= run_test<2, int32_t, 1, sycl::image_channel_type::signed_int32,
  //                   sycl::image_channel_order::r, class int32_1d>({64, 32},
  //                                                                 {4, 2}, 0);


  // hang   -- UR_RESULT_ERROR_OUT_OF_RESOURCES
  // printString("int32_t 2D -- int32_2d");
  // valid &= run_test<3, int32_t, 2, sycl::image_channel_type::signed_int32,
  //                   sycl::image_channel_order::rg, class int32_2d>(
  //     {64, 32, 64}, {4, 2, 4}, 0);


  // hang
  // printString("int16_t 1D -- int16_1d");
  // valid &= run_test<3, int16_t, 1, sycl::image_channel_type::signed_int16,
  //                   sycl::image_channel_order::r, class int16_1d>({64, 32, 64},
  //                                                                 {4, 2, 4}, 0);
