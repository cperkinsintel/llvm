/*
  ...
  clang++ -fsycl -o vsw_2d_test.exe vulkan_sycl_image_interop_write_2d_unsampled.cpp  -lvulkan-1 -I$VULKAN_SDK/Include -L$VULKAN_SDK/Lib
        ...
        sycl::event kernelEvent = q.submit([&](sycl::handler& h) {
            h.parallel_for(sycl::range<2>(width, height), [=](sycl::item<2> item) {
                ...
                if (channels == 1) {
                    ...
                    Vec4 px(generateTestValue<T>(index,0,totalPixels), generateTestValue<T>(index,1,totalPixels), 
                            generateTestValue<T>(index,2,totalPixels), generateTestValue<T>(index,3,totalPixels));
                    syclexp::write_image(unsampledHandle, sycl::int2(x,y), px);
                }
            });
        });
...
int main(int argc, char** argv) {
    ...
    for(int i=1; i<argc; ++i) {
        ...
        if(arg == "--sampled") {
            ...
            return 1;
             ...
             try { width = std::stoi(arg.substr(0, xPos)); height = std::stoi(arg.substr(xPos+1)); } catch(...) {}
        }
    }
    ...
    if (type == "unorm8") return runTest<uint8_t>(width, height, channels, useLinear, useSemaphores, getUnorm8Format(channels), sycl::image_channel_type::unorm_int8); 
    return 1;
}