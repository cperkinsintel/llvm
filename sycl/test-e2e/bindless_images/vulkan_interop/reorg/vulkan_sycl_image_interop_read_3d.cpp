/*
  Unified Vulkan/SYCL 3D Image Read Test (Sampled + Unsampled)
  ...
  clang++ -fsycl -o vsr_3d_test.exe vulkan_sycl_image_interop_read_3d.cpp -lvulkan-1 -I$VULKAN_SDK/Include -L$VULKAN_SDK/Lib
...
// ---------------------------------------------------------
template <typename T>
int runTest(int width, int height, int depth, int channels, bool useLinear, 
    ...
    // 4. SYCL Import and Verification
    ...
    try {
        ...
        if (useSampled) {
            ...
            sampledHandle = syclexp::create_image(devHandle, sampler, imgDesc, q.get_device(), q.get_context());
        } else {
            // Unsampled image
            unsampledHandle = syclexp::create_image(devHandle, imgDesc, q.get_device(), q.get_context());
        }
            ...
            h.parallel_for(sycl::range<3>(width, height, depth), [=](sycl::item<3> item) {
                ...
                if (isUnorm) {
                    ...
                    if (useSampled) {
                        ...
                        px = syclexp::sample_image<Vec4>(sampledHandle, sycl::float3(coordX, coordY, coordZ));
                    } else {
                        // Unsampled: use int coordinates
                        px = syclexp::fetch_image<Vec4>(unsampledHandle, sycl::int3(x, y, z));
                    }
                ...
                if (useSampled) {
                    ...
                    if (channels == 1) {
                        outAcc[baseIdx] = syclexp::fetch_image<T>(unsampledHandle, sycl::int3(x, y, z));
                    } else if (channels == 2) {
        ...
        if (passed) {
            std::cout << "SUCCESS! All " << totalValues << " values match." << std::endl;
        } else {
            std::cout << "FAILURE! " << errorCount << " errors out of " << totalValues << " values." << std::endl;
        }
...
int main(int argc, char** argv) {
    ...
    if (type == "float")  return runTest<float>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    if (type == "half")   return runTest<sycl::half>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    
    if (type == "int32")  return runTest<int32_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    if (type == "uint32") return runTest<uint32_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    
    if (type == "int16")  return runTest<int16_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    if (type == "uint16") return runTest<uint16_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    
    if (type == "uint8")  return runTest<uint8_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled);
    if (type == "int8")   return runTest<int8_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled);

    if (type == "unorm8") {
        return runTest<uint8_t>(width, height, depth, channels, useLinear, useSemaphores, useSampled,
