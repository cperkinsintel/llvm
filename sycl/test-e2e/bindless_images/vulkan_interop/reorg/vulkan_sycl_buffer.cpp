...
inline BufferResources createExportableBuffer(VulkanContext& ctx, VkDeviceSize size, VkBufferUsageFlags usage, VkExternalMemoryHandleTypeFlagBits handleType) {
    ...
    VkExternalMemoryBufferCreateInfo extMemInfo = {VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_BUFFER_CREATE_INFO};
...
inline int getExportFd(VulkanContext& ctx, VkDeviceMemory memory, VkExternalMemoryHandleTypeFlagBits handleType) {
...
void uploadBufferData(VulkanContext& ctx, BufferResources& dst, const std::vector<uint32_t>& data) {
    VkDeviceSize dataSize = data.size() * sizeof(uint32_t);
    
    VkBuffer stagingBuffer; VkDeviceMemory stagingMemory;
    VkBufferCreateInfo bi = {VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    bi.size = dataSize; bi.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
    ...
    void* mapped; vkMapMemory(ctx.device, stagingMemory, 0, dataSize, 0, &mapped);
...
std::vector<uint32_t> downloadBufferData(VulkanContext& ctx, BufferResources& src, size_t count) {
...
int main(int argc, char** argv) {
    ...
    for(int i=1; i<argc; ++i) {
        ...
        if(arg == "--dmabuf") useDmaBuf = true;
        if(arg == "--size" && i+1 < argc) numElements = std::stoi(argv[++i]);
    }

#ifdef _WIN32
    if(useDmaBuf) {
        std::cerr << "WARNING: --dmabuf ignored on Windows. Using Opaque Win32 Handles." << std::endl;
        useDmaBuf = false;
    }
    VkExternalMemoryHandleTypeFlagBits currentHandleType = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT;
#else
    VkExternalMemoryHandleTypeFlagBits currentHandleType = useDmaBuf 
        ? VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT 
        : VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT;
#endif

    std::cout << "Running SYCL Vulkan Buffer Test" << std::endl;
    std::cout << "Elements: " << numElements << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") 
    ...
    BufferResources inBuf = createExportableBuffer(vkCtx, bufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, currentHandleType);
    BufferResources outBuf = createExportableBuffer(vkCtx, bufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, currentHandleType);
    ...
    try {
        ...
        // A. Import Input Buffer
        #ifdef _WIN32
            HANDLE inHandle = getMemHandle(vkCtx, inBuf.memory);
            syclexp::external_mem_descriptor<syclexp::resource_win32_handle> inDesc{inHandle, syclexp::external_mem_handle_type::win32_nt_handle, bufferSize};
        #else
            int inFd = getExportFd(vkCtx, inBuf.memory, currentHandleType);
            auto syclHandleType = useDmaBuf ? syclexp::external_mem_handle_type::dma_buf : syclexp::external_mem_handle_type::opaque_fd;
            syclexp::external_mem_descriptor<syclexp::resource_fd> inDesc{inFd, syclHandleType, bufferSize};
        #endif
        syclexp::external_mem inExtMem = syclexp::import_external_memory(inDesc, device, context);
        
        // B. Import Output Buffer
        #ifdef _WIN32
            HANDLE outHandle = getMemHandle(vkCtx, outBuf.memory);
            syclexp::external_mem_descriptor<syclexp::resource_win32_handle> outDesc{outHandle, syclexp::external_mem_handle_type::win32_nt_handle, bufferSize};
        #else
            int outFd = getExportFd(vkCtx, outBuf.memory, currentHandleType);
            syclexp::external_mem_descriptor<syclexp::resource_fd> outDesc{outFd, syclHandleType, bufferSize};
        #endif
        ...
        if (useSemaphores) {
            #ifdef _WIN32
                auto waitDesc = syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{getSemaphoreHandle(vkCtx, signalSem), syclexp::external_semaphore_handle_type::win32_nt_handle};
                auto signalDesc = syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{getSemaphoreHandle(vkCtx, waitSem), syclexp::external_semaphore_handle_type::win32_nt_handle};
            #else
                auto waitDesc = syclexp::external_semaphore_descriptor<syclexp::resource_fd>{getSemaphoreFd(vkCtx, signalSem), syclexp::external_semaphore_handle_type::opaque_fd};
                auto signalDesc = syclexp::external_semaphore_descriptor<syclexp::resource_fd>{getSemaphoreFd(vkCtx, waitSem), syclexp::external_semaphore_handle_type::opaque_fd};
            #endif
            extWaitSem = syclexp::import_external_semaphore(waitDesc, device, context);
            extSignalSem = syclexp::import_external_semaphore(signalDesc, device, context);
        }
    ...
    for(size_t i=0; i<numElements; ++i) {
        ...
        if(result[i] != expected) {
            if(errors++ < 10) std::cerr << "Mismatch at " << i << " Got " << result[i] << " Exp " << expected << std::endl;
        }
    }

    if (errors == 0) std::cout << "SUCCESS! All " << numElements << " elements verified." << std::endl;
    else std::cout << "FAILURE! " << errors << " mismatches." << std::endl;
