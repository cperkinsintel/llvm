/*
  Unified Vulkan/SYCL 2D Image Read Test (Sampled + Unsampled)
  ...
  clang++ -fsycl  -o vsr_2d_test.exe vulkan_sycl_read_2d.cpp  -lvulkan-1 -I$VULKAN_SDK/Include -L$VULKAN_SDK/Lib
  ...
  NOTE: --linear is not currently working with 2D images.
*/
            ...
            h.parallel_for(sycl::range<2>(width, height), [=](sycl::item<2> item) {
                ...
                if (useSampled) {
                    ...
                    if constexpr (std::is_floating_point_v<T> || std::is_same_v<T, sycl::half>) {
                        ...
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels >= 4) {
                    ...
                    if (channels == 1) {
                        ...
                        sycl::vec<T, 2> pixel = syclexp::fetch_image<sycl::vec<T, 2>>(unsampledHandle, coords);
                        outAcc[base + 0] = pixel.x();
                        outAcc[base + 1] = pixel.y();
                    } else if (channels == 4) {
                        sycl::vec<T, 4> pixel = syclexp::fetch_image<sycl::vec<T, 4>>(unsampledHandle, coords);
                        outAcc[base + 0] = pixel.x();
                        outAcc[base + 1] = pixel.y();
            ...
            if(!checkValue(hostAcc[i], expected)) {
                if (errorCount < 10) {
                    std::cerr << "Mismatch at index " << i 
                              << " (pixel " << pixelIdx << ", channel " << channelIdx << "): "
                              << "Expected " << expected 
