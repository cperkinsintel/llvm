...
__SYCL_EXPORT external_mem import_external_memory<resource_win32_name>(
  ...
  if (externalMemDesc.external_resource.device) {
    ...
    throw sycl::exception(
        ...
        "D3D12 device required in resource_win32_name descriptor");
  }

  // Convert to resource_win32_handle descriptor and use existing implementation
  ...
  // Track the opened handle so we can close it later
  {
    std::lock_guard<std::mutex> lock(g_openedHandlesMutex);
    g_openedHandles[result.raw_handle] = openedHandle;
  }
...
__SYCL_EXPORT external_semaphore import_external_semaphore(
  ...
  if (externalSemaphoreDesc.external_resource.device) {
    ...
    if (!openedHandle) {
      throw sycl::exception(sycl::make_error_code(sycl::errc::invalid),
                            "Failed to open named Win32 semaphore handle");
    }
  } else {
    throw sycl::exception(
        sycl::make_error_code(sycl::errc::invalid),
  ...
  {
    std::lock_guard<std::mutex> lock(g_openedHandlesMutex);
    g_openedHandles[reinterpret_cast<ur_exp_external_mem_handle_t>(
...
#if defined(_WIN32) || defined(_WIN64)
  // Clean up any opened named handles
  {
    std::lock_guard<std::mutex> lock(g_openedHandlesMutex);
