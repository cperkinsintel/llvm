...
// Note: This file provides optimized implementations that use fast kernel cache.
// The header provides inline fallbacks using get_kernel_bundle for backwards compatibility.
//
// To use the optimized path, explicitly instantiate for your kernel types.
// See bottom of this file for examples.
...
// Uses getOrCreateKernel for O(1) cache lookup instead of O(N) device image search
...
get_kernel_info_impl(const context &Ctx, const device &Dev) {
  ...
  sycl::detail::DeviceKernelInfo &DKI = sycl::detail::getDeviceKernelInfo<KernelName>();
  ...
  if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::work_group_size>) {
    size_t result;
    ...
    return result;
  } else if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::compile_work_group_size>) {
    size_t result[3];
    Adapter.call<sycl::detail::UrApiKind::urKernelGetGroupInfo>(
        Kernel, Device,
  ...
  } else if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::preferred_work_group_size_multiple>) {
    size_t result;
    Adapter.call<sycl::detail::UrApiKind::urKernelGetGroupInfo>(
        Kernel, Device,
        ...
        sizeof(size_t), &result, nullptr);
    return result;
  } else if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::private_mem_size>) {
    size_t result;
    Adapter.call<sycl::detail::UrApiKind::urKernelGetGroupInfo>(
        Kernel, Device,
        ...
        sizeof(size_t), &result, nullptr);
    return result;
  } else if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::max_sub_group_size>) {
    uint32_t result;
    ...
    return result;
  } else if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::compile_num_sub_groups>) {
    uint32_t result;
    Adapter.call<sycl::detail::UrApiKind::urKernelGetSubGroupInfo>(
        Kernel, Device,
        ...
        sizeof(uint32_t), &result, nullptr);
    return result;
  } else if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::compile_sub_group_size>) {
    uint32_t result;
    Adapter.call<sycl::detail::UrApiKind::urKernelGetSubGroupInfo>(
        Kernel, Device,
