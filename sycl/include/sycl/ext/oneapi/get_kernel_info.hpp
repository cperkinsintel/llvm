...
get_kernel_info(const context &Ctx, const device &Dev) {
  ...
  if constexpr (std::is_same_v<Param, sycl::info::kernel_device_specific::work_group_size>) {
    ...
    return sycl::detail::getKernelPreferredWorkGroupSizeMultiple(CtxImpl, DevImpl, DKI);
...
get_kernel_info(const context &ctxt, const device &dev) {
  ...
  // Free functions have compiler-generated kernel types that are harder to access
  // For now, use the existing get_kernel_bundle path
  auto Bundle = sycl::ext::oneapi::experimental::get_kernel_bundle<
      Func, sycl::bundle_state::executable>(ctxt, {dev});
  return Bundle.template ext_oneapi_get_kernel<Func>().template get_info<Param>(dev);
}
